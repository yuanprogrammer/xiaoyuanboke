...
import com.xiaoyuan.back.db.dao.CategoryMapper;
import com.xiaoyuan.back.service.CategoryService;
import com.xiaoyuan.back.vo.CategoryNameVo;
import com.xiaoyuan.back.vo.CategoryParentVo;
...
public class CategoryServiceImpl extends ServiceImpl<CategoryMapper, Category> implements CategoryService {
    ...
    public R insert(CategoryParam categoryParam) {
        if (categoryParam == null) return R.error(HttpStatusEnum.PARAM_ERROR);
        ...
        return categoryMapper.insert(category) == 0 ? R.error(HttpStatusEnum.ILLEGAL_OPERATION) : R.ok();
    }

    @Override
    public R modify(Category category) {
        if (category == null || category.getId() == null) return R.error(HttpStatusEnum.PARAM_ERROR);
        return categoryMapper.updateById(category) == 0 ? R.error() : R.ok();
    }

    @Override
    public R move(Category category) {
        if (category == null || category.getId() == null || category.getParentId() == null) return R.error(HttpStatusEnum.PARAM_ERROR);
        Category category1 = new Category();
        category1.setId(category.getId());
        category1.setParentId(category.getParentId());
        return this.baseMapper.updateById(category1) == 0 ? R.error(HttpStatusEnum.ILLEGAL_OPERATION) : R.ok();
    }
    ...
    public R findParentCategoryAll() {
        ...
        return R.ok().data("parentList", this.baseMapper.selectList(wrapper));
    }
