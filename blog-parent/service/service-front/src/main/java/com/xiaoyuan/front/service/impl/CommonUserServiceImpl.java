...
public class CommonUserServiceImpl extends ServiceImpl<CommonUserMapper, CommonUser> implements CommonUserService {
    ...
    public R login(LoginParam loginParam) {
        if (loginParam == null) return R.error(HttpStatusEnum.PARAM_ERROR);
        ...
        }else if (StringUtil.isContainsEmoji(account) || StringUtil.isContainsChinese(account)) {
            return R.error(HttpStatusEnum.PARAM_ILLEGAL);
        }else if (StringUtil.isContainsEmoji(password) || StringUtil.isContainsChinese(password)) {
            return R.error(HttpStatusEnum.PARAM_ILLEGAL);
    ...
    public R register(LoginParam loginParam) {
        ...
        // 参数非法校验
        if (StringUtil.isContainsEmoji(account) || StringUtil.isContainsChinese(account)) {
            return R.error(HttpStatusEnum.PARAM_ILLEGAL);
        }else if (StringUtil.isContainsEmoji(password) || StringUtil.isContainsChinese(password)) {
            return R.error(HttpStatusEnum.PARAM_ILLEGAL);
        }else if (StringUtil.isContainsEmoji(passwordConfirm) || StringUtil.isContainsEmoji(passwordConfirm)) {
            return R.error(HttpStatusEnum.PARAM_ILLEGAL);
        ...
        switch (select) {
            case "1":
                ...
                if (!StringUtil.checkUsername(account)) {
                    return R.error(HttpStatusEnum.PARAM_ERROR);
                }
            ...
            case "2":
                ...
                if (!redisTemplate.hasKey(account)) {
                    return R.error(HttpStatusEnum.CODE_INVALID); // 失效
                }else {
                    if (!code.equals(redisTemplate.opsForValue().get(RedisConstantKey.EMAIL + account))) {
                        return R.error(HttpStatusEnum.CODE_ERROR); // 不正确
                    }
                }
                // 清除验证码
            ...
            case "3":
                if (account.length() != 11) {
                    return R.error(HttpStatusEnum.PARAM_LENGTH_BEYOND);
                }else if (!StringUtil.checkPhone(account)) {
                ...
                if (StringUtils.isBlank(code) || code.length() != 6) {
                    ...
                    if (StringUtils.isBlank(rightCode)) {
                        return R.error(HttpStatusEnum.CODE_INVALID);
    ...
    public R modifyPassword(LoginParam loginParam) {
        ...
        if (StringUtils.isAnyBlank(password, passwordConfirm)) {
            return R.error(HttpStatusEnum.PARAM_ERROR);
        }else if (StringUtil.isContainsEmoji(password) || StringUtil.isContainsEmoji(passwordConfirm)) {
            return R.error(HttpStatusEnum.PARAM_ILLEGAL); // 检测表情符号
        }else if (StringUtil.isContainsChinese(password) || StringUtil.isContainsChinese(passwordConfirm)) {
            return R.error(HttpStatusEnum.PARAM_ILLEGAL); // 检测中文
        ...
        if (commonUser == null) {
            ...
            return this.baseMapper.updateById(commonUser) == 0 ? R.error(HttpStatusEnum.UNKNOWN_ERROR) : R.ok();
        }
    }
    ...
    public R modifyNickname(String token, CommonUserVo commonUserVo) {
        // 参数校验
        if (commonUserVo == null) return R.error(HttpStatusEnum.PARAM_ERROR);
        ...
        if (StringUtils.isBlank(nickname)) {
            return R.error(HttpStatusEnum.PARAM_ERROR);
        }else if (StringUtil.isContainsEmoji(nickname)) {
            return R.error(HttpStatusEnum.PARAM_ILLEGAL);
        ...
        if (!StringUtils.isBlank(username)) {
            if (StringUtil.isContainsEmoji(username)) {
                return R.error(HttpStatusEnum.PARAM_ILLEGAL);
    ...
    public R findPassword(String token, LoginParam loginParam) {
        ...
        if (commonUserVo == null) return R.error(HttpStatusEnum.USER_NO_LOGIN);
        ...
        if (StringUtil.checkEmail(account)) {
            ...
            if (StringUtils.isAnyBlank(rightCode)) {
                return R.error(HttpStatusEnum.CODE_ERROR); // redis中不存在
            }else if (!code.equals(rightCode)) {
                return R.error(HttpStatusEnum.CODE_ERROR); // 不一致
            }
    ...
    public R modifyEmail(String token, CodeParam codeParam) {
        if (codeParam == null) return R.error(HttpStatusEnum.PARAM_ERROR);
        ...
        if (StringUtils.isAnyBlank(token, email, code) || code.length() != 6) {
            return R.error(HttpStatusEnum.PARAM_ERROR);
        }else if (StringUtil.isContainsEmoji(email) || StringUtil.isContainsChinese(email)) {
            return R.error(HttpStatusEnum.EMAIL_ERROR);
        }
        ...
        if (commonUserVo == null) {
            return R.error(HttpStatusEnum.TOKEN_INVALID);
        }
    ...
    public R modifyMobileNumber(String token, CodeParam codeParam) {
        ...
        if (StringUtils.isBlank(token) || StringUtils.isBlank(mobileNumber) || StringUtils.isBlank(code)) {
            ...
            return R.error(HttpStatusEnum.PARAM_ERROR);
        }else if (!StringUtil.checkPhone(mobileNumber)) {
            return R.error(HttpStatusEnum.MOBILE_NUMBER_ERROR);
        }else if (this.baseMapper.isContainsMobileNumber(mobileNumber)) {
            // 号码已被注册
            return R.error(HttpStatusEnum.MOBILE_NUMBER_ALREADY_EXIST);
        }
    ...
    public R getUserInfo(String token) {
        ...
        if (commonUserVo == null) {
            ...
            return R.ok().data("userInfo", commonUserVo);
        }
    }
