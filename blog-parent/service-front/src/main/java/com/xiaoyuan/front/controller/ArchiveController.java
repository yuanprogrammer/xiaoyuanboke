...
public class ArchiveController {
    ...
    public String toArchivePage(Model model, HttpServletRequest request, HttpServletResponse response) {
        ...
        String HTML = redisTemplate.opsForValue().get("archive");
        ...
        HTML = thymeleafViewResolver.getTemplateEngine().process("archive/archive", webContext);
        if (!StringUtils.isEmpty(HTML)) {
            // 丢入redis缓存
            redisTemplate.opsForValue().set("archive", HTML);
        }
        return HTML;
    }
}
